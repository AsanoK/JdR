...
\section{Session 21}
\subsection{Préparations}
On veut réduire légèrement la complexité de la campagne, tout en épaississant les plots existant.

Après un retour sans histoire, et quelques éléments de présentation, on pourrait amener la secte à intervenir à nouveau, vite et fort, dans des scènes qui impliquent au passage le fiancé de notre investigatrice par exemple.

recontre 1 : cultistes de base :
4 cultists : 60xp p229 GMG (15xp chacun)
1 necromancer : 60 XP p230 GMG

modifs : 
sentinelles archers : 20xp chacun
executioner : 80xp
burglar : 40xp
\subsection{Déroulement}
Retour de l'équipe à la capitale, courses et échange d'informations avec la marcahnde Ysoki.

Un messager est arrivé pour indiquer que c'était le chaos, avec des types tuant à tour de bras près de a porte nord.

Il y a effectivement une vingtaine de cultistes (-4 après un premier combat avec les PJs).

Butin : armure cuir clouté +1 et potion d'invisibilité
\section{Sessions 22-25}
Les cultistes de la porte nord ont été tués, mais pas avant d'avoir enlevé le fiancé de l'investigatrice.

Ils ont été poursuivis hors de la ville, jusqu'à un petit domaine à l'abandon, où se trouvait un repaire important, nettoyé par les joueuses. Elles apprennent que les cultistes cherchent les "héritiers", dont ferait partie Kil Ae. Des lettres ont été retrouvées sur place.

De retour à la capitale, chasse à la Guenaude, en passant par le marché noir de la ville, pour y trouver des informations. La guenaude finit morte, le manoir qu'elle occupait étant intégralement nettoyé par les PJs. D'autres lettres ont été trouvées, certaines chiffrées, d'autres simulant clairement plusieurs communications avec des personnalités différentes.

Celles-ci ont également déchiffré les lettres des cultistes, qui indiquent que leur repaire le plus sacré est à proximité du 4ème vieillard, dans le nord du royaume. Les PJs se préparent à y aller.
\section{Session 23 (et plus?)}
\subsection{Préparation}
Objectifs : faire voyager les joueuses dans le royaume, en traversant une bonne partie de celui-ci : par la route vers Chowon, puis par la rivière jusqu'à la forêt des esprits. Ce voyage est entrepris au milieu de l'automne, et va prendre un certain temps (retour probable en hiver).

Evènements prévus:
\begin{itemize}
\item Croiser des dragons sauvages, y compris le dragon protecteur de la rivière, peut-être après une attaque  de 7 lacedons, dans un secteur qui connaît des problèmes de morts-vivants. Quête secondaire : purger les non-morts du vieux sanctuaire, où un esprit en colère rôde désormais (wraith, lvl6). Le gardien de la rivière, Hantan, est un puissant dragon, ancien, qui va aller renouveler son serment au roi lors du grand festival du changement d'année. Il sera curieux d'avoir des informations sur son état.
\item les paysans du coin montrent le respect le plus absolu pour elles, devant leur rang impressionnant. Montrer ce respect dans les attitudes des personnages. Arrêts en taverne relativement fréquents, puis négociation d'un bateau notamment.
\item Dans les montagnes du nord, probablement les faire croiser au moins une patrouille de la bannière blanche. Peut-être dans le domaine familial de la noble de l'équipe. Pas des troupes connues, mais elles pourraient avoir une utilité plus tard dans la campagne ?
\item Le domaine de la famille Shin : un domaine relativement fortifié, à la limite de la forêt des esprits. Celui-ci n'est pas d'une richesse incroyable, mais profite notamment de la proximité de la forêt (et de ses ressources magiques). Shin a pas mal de questions à sa famille, qui n'a pas nécessairement les réponses d'ailleurs.
\item La forêt des esprits, où on peut vraiment jouer sur le côté Mononoke : rencontres un peu oniriques avec le souverain de ces bois (compte comme une fée), piège tendu par un esprit plus retord que les autres (un oni ?).
\end{itemize}

...
Dans S\&A les dés utilisés sont tous des dés à six faces, ou hexahèdres, ou plus simplement des d6. Ainsi, si le MJ demande de lancer 5d6 dans le cadre d’un test, il s’agit de lancer 5 décaèdres.
...
%\section{Séquelles des blessures}
%Pertinence ?
...
\chapter{Combats}
\section{Généralités:}
Dans un combat, les participants agissent par ordre d’initiative décroissante. L’initiative étant calculée avec 1d6+VIT.
...
%-manipulation élémentaire (terre, feu, air, eau, ....) : permet de manipuler les mouvements d'un type d'élément. La difficulté du tout dépend de la masse à déplacer, de la distance entre celle-ci et le rituel, et certains matériaux (le fer, encore le fer) y résistent naturellement, d'autres y sont très sensibles, comme les cristaux (beaucoup plus rares). Enchantement+CON
...
%-condition : modifie un rituel existant pour lui ajouter une conditions de déclenchement. Lorsque celles-ci sont réalisées, le sort se déclenche, et ce, tant que le réceptacle existe. Un réceptacle est un objet servant de lien entre le lanceur et le sort inactif. Il est possible pour quelqu'un de suivre ce lien. Enchantement+CON
...
%cérémonies courantes (équivalent messes dominicales) : facile, même pour beaucoup de monde, une fois/semaine, bonus léger (relance d'un jet dans la semaine). TEST DE LITURGIE+SOC assez simple
...
%cérémonies de bénédiction : plus difficile, notamment pour beaucoup de monde. Permet une relance d'un test lié à la bénédiction par jour (plus la bénédiction est longue, plus c'est compliqué de la faire fonctionner). =>Peut-être un autre bonus, plus important, ou du moins plus présent? TEST DE LITURGIE + SOC, modifié par le nombre de gens/objets affectés.
...
%Autre bonus : armes bénies contre certaines entités ?
\section{Rites}
%question à poser à une divinité, ou demande spécifique. Malus important en cas de demandes régulières et/ou futiles.
%Permet d'avoir une vision plus ou moins pertinente (mais très probablement cryptique), ou de recevoir rapidement un signe de la divinité (qu'il faudra sans doute interpréter évidemment, avec un test de théologie). Les symboles courants du culte seront fréquemment utilisés dans ces visions et signes mystiques. TEST DE RITE+CON, modifié par le nombre de question et le temps depuis la dernière.
